...
@else´
    @extends('layouts.app')
    @section('title')
        <h1 class="titulo"><i class="fa fa-wpforms"></i> Módulo pruebas saber pro</h1>
        @section('message')<p>Lista de registro pruebas saber pro</p>@endsection
    @endsection
    @section('content')
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-7">
                            ...
                            <a class="btn btn-outline-danger btn-radius" href="{{ url('prueba/pdf') }}"
                                title="Generar reporte pdf" target="_blank"><i class="fa fa-file-pdf-o"></i></a>
                            <a class="btn btn-outline-primary btn-radius" href="{{ url('prueba/export') }}"
                                title="Generar reporte excel" target="_blank"><i class="fa fa-file-excel-o"></i></a>
                        </div>
                        <div class="col-md-5 d-flex justify-content-end align-items-center">
                            @if (Auth::user()->per_tipo_usuario == 1 || Auth::user()->per_tipo_usuario == 2)
                                <a class="btn btn-outline-success " href="{{ url('prueba/create') }}"><i
                        ...
                        <table class="display mt-4 p-3" id="tables">
                            ...
                            <tbody>
                                ...
                                @foreach ($pruebas as $prueba)
                                    <tr>
                                        <td>{{ $i++ }}</td>
                                        <td>{{ $prueba->estudiantes->estu_nombre . ' ' . $prueba->estudiantes->estu_apellido }}
                                        </td>
                                        ...
                                        @if (Auth::user()->per_tipo_usuario == 1 || Auth::user()->per_tipo_usuario == 2)
                                            <td>
                                                <form action="{{ route('prueba.destroy', $prueba->id) }}" method="POST">
                                                    <div class="d-flex">
                                                        <a class="btn btn-sm" href="/prueba/{{ $prueba->id }}"><i
                                                        class="fa fa-folder-open-o"></i></a>
                                                        <a class="btn btn-outline-info btn-sm   "
                                                            href="/prueba/{{ $prueba->id }}/edit"><i
